...
#' @concept targetscore
#' @export
my_calc_targetscore <- function(wk, wks, dist_ind, edgelist, n_dose, n_prot, proteomic_responses, fs_dat,
  ...
  for (i in 1:n_dose) {
    for (j in 1:n_prot) {
        prot_name = colnames(proteomic_responses)[j]
        #print(prot_name)
        
        tsd[i, j] <- fs[prot_name, 'fs'] * (proteomic_responses[i, j] + sum(tsp[i, 1:n_prot, j]))
    }
  }
